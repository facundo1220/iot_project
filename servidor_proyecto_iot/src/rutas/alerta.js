...
router.get('/alerta', (req, res) => {
  ...
  connection.getConnection(function(error, tempConn) { //conexion a mysql
    if (error) {
      ...
      tempConn.query('SELECT * FROM alerta', function(error, result) {
        ...
        if (error) {
          ...
          for (i = 0; i < resultado.length; i++) { //se lee
            json1 = {
              "idalerta": resultado[i].idalerta,
              "estado": resultado[i].estado,
              "valor": resultado[i].valor,
              "timestamp_alerta": resultado[i].timestamp_alerta,
              "bebeid": resultado[i].bebeid
            };
  ...
  connection.getConnection(function(error, tempConn) { //conexion a mysql
    if (error) {
      ...
      tempConn.query('INSERT INTO alerta VALUES(null, ?, ?,?,?)',
        [json1.estado, json1.valor,json1.timestamp_alerta, json1.bebeid],
...
router.delete('/alerta', (req, res) => {
  ...
  connection.getConnection(function(error, tempConn) { //conexion a mysql
    if (error) {
      ...
      var del="DELETE FROM alerta WHERE ";
...
router.put('/alerta', (req, res) => {
  ...
  connection.getConnection(function(error, tempConn) { //conexion a mysql
    if (error) {
      ...
      var put="UPDATE alerta SET ";
      var key=Object.keys(json1);
      var value=Object.values(json1);

      var comand=put + key[0] + "='" + value[0] + "'," + key[1] + "=" + value[1] + "," + key[2] + "='" + value[2] + "'," + key[3] + "=" + value[3] + " WHERE " +  key[4] + "=" + value[4];
